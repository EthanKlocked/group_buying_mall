...
const Auth = () => {
    ...
    const passLevel = async() => {
        ...
        if(editor){
            ...
            const result = await apiCall.post("/join", {params}); //----JOIN----//

            if(result.data.res == 'SUCCESS' && result.data.id){
                //point process            
                apiCall.put("/member/any/point", { type : 'join', id: result.data.id, phone: state.certPhone }); //for user joining
                if(hostId) apiCall.put("/member/any/point", { type : 'invite', id: hostId, phone: state.certPhone });  //for user inviting

                //login
                const loginResult = await apiCall.get(`/login/${result.data.id}/phone`); //s----LOGIN----//
