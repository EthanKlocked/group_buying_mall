...
const Login = React.memo(() => {
    ...
    const kakaoAuth = async() => {
        const inviteId = hostId || ''; 

        //use sdk for mobile only
        if(isMobile) return Kakao.Auth.authorize({redirectUri: `${KAKAO_REDIRECT}`, state:inviteId});
        
        //setting
        const kakaoLoginUrl = `https://kauth.kakao.com/oauth/authorize?response_type=code&client_id=${KAKAO_API_KEY}&redirect_uri=${KAKAO_REDIRECT}&state=${inviteId}`;

        //order set
        if(order && order.set) window.localStorage.setItem('orderSet', JSON.stringify(order.set));

        //kakao login open [iframe]
        if(window.self != window.top) return window.parent.postMessage({type:"kakao", loginUrl:kakaoLoginUrl}, `${process.env.REACT_APP_HOST}`);
        ...
        return (
            ...
            <StyledContainer>
                ...
                <StyledContent>
                    ...
                    <Link to={`/Login/Phone${extraParams}`}>
                        <StyledButton>
